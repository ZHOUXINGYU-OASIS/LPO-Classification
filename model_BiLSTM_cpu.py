# -*- coding: utf-8 -*-
"""
Created on Wed Jan  4 21:32:14 2023
...
class Model_LeakyReLU(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, output_size):
        ...
        self.lstm = nn.LSTM(input_size=self.hidden_size, hidden_size=self.hidden_size, num_layers=self.num_layers,
                            bidirectional=False)
        self.state0 = torch.zeros([1, self.input_size])
    ...
    def forward(self, obs):
        ...
        c0 = torch.zeros([self.num_directions * self.num_layers, self.hidden_size])

        obs = self.obs_embedding(obs).view(-1, self.hidden_size)
        output, _ = self.lstm(obs, (h0, c0))

        state = self.state_decoding(output)
        # state = state[:,-1, :]
        return state
